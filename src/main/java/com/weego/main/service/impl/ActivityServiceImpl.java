...
public class ActivityServiceImpl implements ActivityService {
	...
	public ActivityDetailDto getActivityDetail(String cityActivityId) {
		...
		try {
			...
			if (cityActivity != null) {
				...
				// 详情页的地址需要拼接
				if (cityActivity.getDetailAddress() != null
						&& cityActivity.getAddress() != null
						&& !cityActivity.getDetailAddress().endsWith(
								cityActivity.getAddress())) {
					activityDetailDto.setDetailAddress(cityActivity
							.getDetailAddress()
							+ ", "
							+ cityActivity.getAddress());
				} else if (cityActivity.getDetailAddress() == null
						&& cityActivity.getAddress() != null) {
					activityDetailDto.setDetailAddress(cityActivity
							.getAddress());
				} else if (cityActivity.getDetailAddress() != null
				...
				List<ActivityParagraphs> cityActivityParagraphsList = cityActivity
						.getParagraphs();

				List<ActivityParagraphsDto> activityParagraphsDtoList = new ArrayList<ActivityParagraphsDto>();

				if (cityActivityParagraphsList != null
						&& !cityActivityParagraphsList.isEmpty()) {
					for (ActivityParagraphs cityActivityParagraphs : cityActivityParagraphsList) {
						...
						activityParagraphsDto.setDetail(cityActivityParagraphs
								.getDetail());
						activityParagraphsDto
								.setImageBrief(cityActivityParagraphs
										.getImageBrief());
						activityParagraphsDto
								.setImageTitle(cityActivityParagraphs
										.getImageTitle());
						activityParagraphsDto
								.setImageUrl(cityActivityParagraphs
										.getImageUrl());
	...
	public List<ActivityBaseDto> getActivityList(String cityId) {
		...
		try {
			List<Activity> activityList = cityActivityDao
					.getAllActivity(cityId);
					...
					if (openTime != null && closeTime != null) {
						...
						try {
							...
							if (openNow <= 7 & openNow > 0) {
								...
								String openMMdd = (openTime.getMonth() + 1)
										+ "月" + openTime.getDate() + "日";
								String closeMMdd = (closeTime.getMonth() + 1)
										+ "月" + closeTime.getDate() + "日";
