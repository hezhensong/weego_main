...
public class OpenTimeUtil {
	...
	public static List<String> changeTimeDesc(List<String> timeDesc) {
		List<String> newDesc = new ArrayList<String>();
		if (timeDesc != null && timeDesc.size() > 0) {
			for (int i = 0; i < timeDesc.size(); i++) {
				if(timeDesc.get(i).contains("星期")) {
					newDesc.add(timeDesc.get(i).replace("星期", "周"));
				} else {
					newDesc.add(timeDesc.get(i));
				}
			}
		} 
		return newDesc;
